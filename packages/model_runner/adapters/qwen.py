...
class QwenAdapter:
    """Import-safe adapter for the Qwen text model.
    ...
    reading its declared :attr:`capabilities` never downloads a model. Qwen is a
    ...
    def _load_runtime(self) -> Any:
        ...
        if not self._model_name:
            raise ValueError(
                "QwenAdapter requires a 'model_name' before generation; "
                "set it in configs/models/qwen.yaml."
            )
    ...
    def generate_text(
        ...
        try:
            ...
            return ModelResponse(error=f"qwen load failed: {exc}")
